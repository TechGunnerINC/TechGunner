...
const put = async (req, res) => {
  try {
    ...
    if (!p) {
      return res
        .status(404)
        .send("Blog not found or you are not authorized to update it");
    }

    return res
      .status(200)
      .json({ message: "Blog was updated!", updatedBlog: p });
  } catch (err) {
    console.error(err.message);
...
const del = async (req, res) => {
  try {
    ...
    if (!delRes) {
      return res
        .status(404)
        .send("Blog not found or you are not authorized to delete it");
    }
...
const edit = async (req, res) => {
  try {
    ...
    if (comments.uid !== uid) {
      return res
        .status(403)
        .send("You are not authorized to update this comments");
    }
...
const destroy = async (req, res) => {
  try {
    ...
    if (comments.uid !== uid) {
      return res
        .status(403)
        .send("You are not authorized to delete this comments");
    }
